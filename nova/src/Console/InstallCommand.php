...
class InstallCommand extends Command
{
    ...
    public function handle()
    {
        ...
        $this->comment('Generating UserModel Resource...');
        $this->callSilent('nova:resource', ['name' => 'UserModel']);
        copy($this->resolveStubPath('/stubs/nova/user-resource.stub'), app_path('Nova/UserModel.php'));

        if (file_exists(app_path('Models/UserModel.php'))) {
            file_put_contents(
                app_path('Nova/UserModel.php'),
                str_replace('App\User::class', 'App\Models\User::class', file_get_contents(app_path('Nova/UserModel.php')))
            );
        }
    ...
    protected function setAppNamespace()
    {
        ...
        $this->setAppNamespaceOn(app_path('Nova/UserModel.php'), $namespace);
